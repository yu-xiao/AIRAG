...
- **score 双语义**(spec 明确):rerank 开启=智谱相关度分(0~1);关闭=RRF 融合分。
  引用构建不依赖 score,不受影响。
- 配置:`RETRIEVAL_MIN_SCORE`,默认 **0.30**(保守),**0 = 禁用**;.env.example/README 同步。
...
- rerank on + 零命中问题 → done 帧 `refused=true` 且 citations 为空;
...
| 风险 | 缓解 |
|---|---|
| 阈值误伤正常问题 | 默认保守 0.30;eval 基线对比;可 env 调 0 禁用 |
...
- 实施期跑 §5 对比后回填:零命中拒答率 / 正常题基线对比 / 终值。
