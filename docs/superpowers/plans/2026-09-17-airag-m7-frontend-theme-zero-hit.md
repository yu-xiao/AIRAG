...
### 待办
- ~~Step 4 用户浏览器双主题走查~~ **2026-09-17 用户走查通过**(见"走查增量")。
- M8 候选(终审分流):包裹型拒答检测加固(**升级**:阈值禁用后提示词门控是主防线)、refused 入 rewrite reset(拓扑护栏测试)、DocsPage 状态图标(spec 有计划无,已裁定接受偏差)、过滤空态文案/?create=1 残留、下拉键盘可达性、暗色 hljs-regexp 对比度一行修、--app-bg-soft 未消费、zhipu null 分防护、eval_sets 残留自动清理、**知识库重名限制/选项区分度**(走查发现)、既有 M8 池(评估入库/多跳并行/MinerU 本地化/LDAP 等)。

## 走查增量(2026-09-17,用户浏览器走查)

用户走查通过,过程中三个增量已处理:
1. **知识库下拉重名困惑**:排查为开发库数据问题(验收复跑产生的重名库 5/6、7/8、9/10),事务级联删除重复库 6/8/10(7 文档/17 分块/7 会话/22 消息),保留各对首次验收库;M7验收库(id 9)与 eval_sets/9.json 完好。"建库不限制重名、多选下拉重名不可区分"记 M8 候选。
2. **collapse-tags "+N" 不直观**(9882746):知识库多选加 `collapse-tags-tooltip`,悬停折叠计数浮出剩余库名。
3. **对话页全面精装**(6b30ef7,+281/-73,纯表现层):空态 Hero(渐变品牌标+时段问候+示例问题 chips 点击填入)、composer 圆角输入卡(focus-within 主色描边/圆形纸飞机发送钮/红色圆形停止键/autosize 2-4 行/快捷键提示下移)、精排开关标签横排修复折行(带说明 tooltip)、侧栏会话项两行排布+图标+相对时间、用户气泡主色渐变非对称圆角、助手气泡卡片底细边框、打字三点品牌色、消息入场微动效(尊重 prefers-reduced-motion)。业务逻辑/SSE 流/refused 渲染零改动;lint 0/0+build 绿+vitest 11P。
